...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	for (int i = 1; i < s.size(); i++)
	{
		Sensor ToInsert = s[i];
		int pos = i;
		while ((pos > 0) && (comp(ToInsert, s[pos - 1])))
		{
			s[pos] = s[pos - 1];
			pos--;
		}
...
int main()
{
	...
	// destruction time.
	{
		...
		//        order.
		for (size_t i = 0; i < SensorsPtr.size(); i++)

...
{
	if (n2 <= 0)  //if the all elements in a2 are found in a1
	{
		return 1;
	}
	else if (n1 <= 0) //if the a2 is not found in a1
	{
		return 0;
	}
	else if (a1[n1 - 1] == a2[n2 - 1]) //if two strings are identical
	{
	    return countIncludes(a1, n1 - 1, a2, n2 - 1) + countIncludes(a1, n1-1, a2, n2); //sum the situations that (1) this situation is valid or that (2) other situations exist
	} 
	else
	{
		return countIncludes(a1, n1 - 1, a2, n2); //not match, go to the next one
	}
}
...
void order(string a[], int n)
{
	if (n>=2)
	{
		int first;
		int last;
		separate(a, n, a[n-1], first, last); //seperate the array to two sections
		order(a, first); //sort the first half
		order(a + last, n - last); //sort the second half
	}
}
